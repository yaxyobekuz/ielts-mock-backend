...
/**
 * Schedule UserStats update for a user
 * This updates cumulative lifetime statistics
 */
const scheduleUserStatsUpdate = async (userId, updateData) => {
  try {
    ...
    console.error("UserStats job schedule xatolik:", error);
  }
};

/**
 * Schedule Stats update for a user (daily stats)
 * This creates/updates daily statistics entry
 */
const scheduleStatsUpdate = async (userId, role, supervisor, updateData) => {
  try {
    await agenda.now("update-stats", {
      role,
      userId: userId.toString(),
      updateData,
      supervisor: supervisor ? supervisor.toString() : null,
    });
  } catch (error) {
    console.error("Stats job schedule xatolik:", error);
  }
};
...
agenda.define("update-user-stats", async (job) => {
  const { userId, updateData } = job.attrs.data;
  ...
  try {
    ...
    if (hasAvgScoreUpdate) {
      ...
      if (userStats) {
        ...
        const newResultsCount = totalResults + (updateData["results.active"] || 0);

        // Recalculate averages
        const modules = ["overall", "reading", "writing", "speaking", "listening"];
        ...
        modules.forEach((module) => {
          ...
          if (newScore !== undefined) {
            ...
            const newAvg = ((currentAvg * totalResults) + newScore) / newResultsCount;
    ...
    console.error(`UserStats update xatolik (userId: ${userId}):`, error);
  }
});
...
agenda.define("update-stats", async (job) => {
  const { userId, role, supervisor, updateData } = job.attrs.data;
  ...
  try {
    ...
    if (hasAvgScoreUpdate) {
      ...
      if (stats) {
        ...
        const newResultsCount = totalResults + (updateData["results.created"] || 0);
    ...
    console.error(`Stats update xatolik (userId: ${userId}):`, error);
  }
});
